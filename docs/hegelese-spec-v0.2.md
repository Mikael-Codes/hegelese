# Hegelese — specification proposal v0.2

**Hic Rhodos, Hic Saltus.**

23 September 2026. Status: language-design proposal, not an implemented language or a completed formal semantics. This revision supersedes the terminology and central design claims of the initial conversational draft. Its philosophical research is provisional: see [reading coverage](reading-coverage.md) and [critical reflection](critical-reflection.md).
...
Its central question is: **When an abstraction cannot meet a stated demand, how can we develop another abstraction and make the relationship between them inspectable?**

A useful answer needs more than a before/after value. It needs the demand, the demonstrated limitation, the construction of the successor, the treatment of earlier commitments, and the evidence supporting each claim.
...
## 4. Three levels that must remain distinguishable
...
| Development | A construction connecting concept specifications and accounting for a specific limitation | Extending addition to support inverses |
...
Version 0.2 must not equate static type checking with proof of the declared equations. A concept implementation can be well typed while a law is false, untested, or assumed.

## 6. Demands and limitations
...
| Addressed limitation | The demand and evidence this construction is intended to address |
...
Version 0.2 proposes runtime checks and explicit assumptions. A trusted general proof checker is future work, so examples must not fabricate `Proved` results.
...
## 13. Becoming, recurrence, and recollection
...
Recollection is a proposed tool for re-expressing an executed development as a dependency structure: what was required, which assumptions changed, and how earlier results appear within the later representation. The original trace remains available. Recollection must not rewrite the chronology or retrospectively label an externally supplied decision as inevitable.
...
An initial implementation should cover the ordinary functional core, exact keyword aliases, explicit concept packages, registered Upheaval constructions, bounded runtime checks, and evidence traces. Advanced effect inference, theorem-prover integration, automatic candidate generation, quotient syntax, and general reflective metaprogramming are deferred.
